...
class EvaluationController {
  ...
  async createEvaluation(req, res) {
    try {
      ...
      console.log('Received evaluation request:', JSON.stringify(evaluationRequest, null, 2));
      ...
      const result = await this.evaluationService.evaluatePrompts(evaluationRequest);
  ...
  async getEvaluationCriteria(req, res) {
    try {
      const criteria = [
        {
          name: 'accuracy',
          description: 'Evaluates factual correctness and proper handling of input',
          enabled: true
        },
        {
          name: 'relevance',
          description: 'Assesses how well the response addresses the user query',
          enabled: true
        },
        {
          name: 'completeness',
          description: 'Checks if all parts of the query are adequately addressed',
          enabled: true
        },
        {
          name: 'format',
          description: 'Verifies adherence to expected output format and structure',
          enabled: false
        },
        {
          name: 'clarity',
  ...
  convertResultsToCSV(results) {
    ...
    const rows = results.results.map(result => {
      const assertionsSummary = result.gradingResult?.componentResults
