...
class OptimizeController {
  ...
  async optimizePrompt(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (
        !originalPrompt ||
        !failedClusters ||
        !Array.isArray(failedClusters)
      ) {
        res.status(400).json({
          success: false,
          error: "Original prompt and failed clusters are required",
          message:
            "Please provide originalPrompt (string) and failedClusters (array)",
        });
        return;
      }
  ...
  async comparePrompts(req: Request, res: Response): Promise<void> {
    try {
      ...
      const comparison = await this.performPromptComparison(
        originalPrompt,
        optimizedPrompt,
        testCases,
      );
  ...
  private extractImprovements(
    ...
    const clusterReasons = failedClusters.map((cluster) =>
      (cluster.reason || "").toLowerCase(),
    );
    ...
    if (
      optimizedPrompt.includes("example") ||
      optimizedPrompt.includes("Example")
    ) {
    ...
    return improvements.length > 0
      ? improvements
      : ["General prompt structure and clarity improvements"];
  }
  ...
  private analyzeFailurePatterns(failedClusters: FailedCluster[]): any {
    ...
    const totalFailedTests = failedClusters.reduce(
      (sum, cluster) => sum + cluster.failedTestCases.length,
      0,
    );
    ...
    failedClusters.forEach((cluster) => {
      ...
      cluster.failedTestCases.forEach((testCase) => {
        ...
        assertionTypes.set(
          assertionType,
          (assertionTypes.get(assertionType) || 0) + 1,
        );
  ...
  private generateRecommendations(
    reasonFrequency: Map<string, number>,
    ...
    if (
      ...
      scoreDistribution.medium + scoreDistribution.high
    ) {
    ...
    const topReasons = Array.from(reasonFrequency.entries()).sort(
      (a, b) => b[1] - a[1],
    );
    ...
    if (topReasons.length > 0) {
      ...
      if (topReason.includes("context")) {
        recommendations.push(
          "Improve context preservation and interpretation instructions",
        );
      }
   ...
   */
  private async performPromptComparison(
    originalPrompt: string,
    optimizedPrompt: string,
    testCases?: any[],
  ...
  private generateOptimizationSuggestions(
    ...
    if (
      !originalPrompt.includes("format") &&
      !originalPrompt.includes("Format")
    ) {
    ...
    return suggestions.length > 0
      ? suggestions
      : ["Consider adding more specific instructions and examples"];
  }
