...
const validatePromptGeneration = [
  ...
  body('techniques')
    ...
    .custom((techniques) => {
      ...
      if (techniques && techniques.some(tech => !validTechniques.includes(tech))) {
...
const validatePromptEvaluation = [
  ...
  body('actualOutput')
    .notEmpty()
    .withMessage('Actual output is required'),
...
const validatePromptfooEvaluation = [
  ...
  body('prompts.*')
    .notEmpty()
    .withMessage('Each prompt must not be empty'),
  ...
  (req, res, next) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      console.log('Validation errors for promptfoo evaluation:', JSON.stringify(errors.array(), null, 2));
